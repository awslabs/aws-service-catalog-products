...
def get_org_role():
    return os.environ.get('ORG_ROLE')
...
def create_policy(event, context):
    ...
    try:
        session = assume_role(get_org_role(), "CreatePolicy")
...
def update_policy(event, context):
    ...
    try:
        ...
        session = assume_role(get_org_role(), "UpdatePolicy")
        client = session.client('organizations')
...
def delete_policy(event, context):
    ...
    try:
        policy_id = event['PhysicalResourceId']
        session = assume_role(get_org_role(), "DeletePolicy")
        client = session.client('organizations')
